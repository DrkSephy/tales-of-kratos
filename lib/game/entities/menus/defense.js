	   	...
	   	clicked: function() {
	        ...
	        console.log("Clicked defense1 icon");
	       	if(ig.game.skillsCatalog['healthIncrease1'].active && ig.game.skillsCatalog['attack1'].active){
		        if(!ig.game.skillsCatalog['defense1'].active && player.abilityPoints >= ig.game.skillsCatalog['defense1'].points){
		        	player.stats.def += ig.game.skillsCatalog['defense1'].increase;
		        	ig.game.skillsCatalog['defense1'].active = true;
		        	player.abilityPoints -= ig.game.skillsCatalog['defense1'].points;
		        }
		    }
	    },
	});
