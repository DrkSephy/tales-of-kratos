        ...
        draw: function(){
            ...
            if(this.gameState === 'artesMenu' && this.menuVisible){
                ...
                if(!this.spawnedArteBoxes){
                    this.spawnEntity(EntityArteBox, 32, 32);
                    this.spawnEntity(EntityIncrease, 120, 40);
                    this.spawnEntity(EntityMinus, 160, 40);
                    this.spawnEntity(EntityArteBox, 32, 80);
                    this.spawnEntity(EntityArteBox, 32, 128);
                    this.spawnEntity(EntityArteBox, 32, 176);
                ...
                if(ig.input.pressed('escape')){
                    ...
                    var boxes = ig.game.getEntitiesByType('EntityArteBox');
                    for(var i = 0; i < boxes.length; i++){
                        boxes[i].kill();
                    }
                    var menu = ig.game.getEntitiesByType('EntityTransMenu')[0];
                    menu.kill();
                    var plus = ig.game.getEntitiesByType('EntityIncrease')[0];
                    plus.kill();
                    var minus = ig.game.getEntitiesByType('EntityMinus')[0];
                    minus.kill();
                }
            }
